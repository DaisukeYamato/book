# setting
setwd("C:/Users/daisuke.yamato/work/book/R_series/3.bayes");

# Kalman filter
data <- read.table("r3/関連データセット/times2.txt");
N <- nrow(data);
y <- data[,1];
...
KF1 <- function(y, XF0, VF0, F, G, Q, R, limy, ISW, OSW, m, q, N) {
	if (OSW == 1){
		XPS <- matrix(0, m, N);
		XFS <- matrix(0, m, N);
		VPS <- array(dim = c(m, m, N));
		VFS <- array(dim = c(m, m, N));
	}
...
# -- init
limy <- 1e20; # lim below for null values
ISW <- 0; k <- 3; R <- diag(1);
